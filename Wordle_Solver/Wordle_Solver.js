...
let aryAllPossibleAnswersSet = new Set(); // companion Set for O(1) membership checks; rebuilt alongside aryAllPossibleAnswers
...
async function initialize() {                                                   // set default selections
  ...
  if (!boolAnswersOnly) {                                           // add all possible Guesses
    const seenWords = new Set(aryAllPossibleAnswers);               // O(1) membership checks instead of O(n) Array.includes() per word
    for (const word of aryAllPossibleGuesses) {
      if (!seenWords.has(word)) {                                   // if the word is not already in the array
        aryAllPossibleAnswers.push(word);                           // add it!
        seenWords.add(word);
      } // if
    } // for
  } // if
  aryAllPossibleAnswersSet = new Set(aryAllPossibleAnswers);        // keep the companion Set in sync for O(1) lookups elsewhere
...
function buildStrFilteredFiveLetterWords(array) {                   // helper function to concatenate strings (words)
  ...
  for (const word of array) {
    ...
    if (isDailyAnswer) {                                            // daily Answer (via API/built-in); bold, italicize and underline word
      strBuilt += '<strong><em><u><span>' + linkStart + linkEnd + '</span></u></em></strong>' + space;
      consoleLog(logAnswers, 'Daily Answer: ' + word);
    } else if (aryAllAnswersOrderedSet.has(word)) {                  // word is a possible Answer
      strBuilt += '<strong><span style="font-size: 90%;">' + linkStart + linkEnd + '</span></strong>' + space;
      if (logAnswers) { consoleLog(logAnswers, 'Answer: ' + word + 'index: ' + aryAllAnswersOrdered.indexOf(word)); }
...
function solveIt() {
    ...
    for (let letterPosition = 1; letterPosition <= 5; letterPosition++) {
      ...
      if ((guessWord !== '     ') && (guessWord !== '') && (guessWord.length === 5)) {
        ...
        if (guessWord === 'HUOMO') {
          ...
          initialize();
          return;                                                   // terminate further processing
      } else if (!((aryAllPossibleGuessesSet.has(guessWord)) || (aryAllPossibleAnswersSet.has(guessWord)))) { // invalid guess word
          ...
          if (logGeneral) { consoleLog(logGeneral, 'aryAllAnswersOrdered.indexOf(guessWord): ' + aryAllAnswersOrdered.indexOf(guessWord)); }
  ...
  for (let word of aryAllPossibleAnswers) {                         // loop through all possible words
    let boolExclude;                                                // true if word excludes all exclude letters
    let boolInclude;                                                // true if word includes any include letter
